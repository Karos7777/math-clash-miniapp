...
Money movement is handled by the Solidity contract. Lobby matching, table UI state, Provably Fair v1 dealing receipts, prototype cards, and chat are stored in Cloudflare KV on Pages, with local JSON storage for development.

Important: Provably Fair v1 improves randomness, but card dealing is still partially off-chain and not full mental poker/ZK. This is fine for Base Sepolia testing, but production poker needs a stronger dealing design.
...
- `revealSeed(bytes32 tableId,uint256 handId,string secret)`: reveal the seed within 60 seconds.
...
- Seed: after two reveals, the backend combines `secret1`, `secret2`, `tableId`, `handId`, chain data, chain id, and the contract address into one final seed.
...
This prevents one player from choosing a seed after seeing the other seed. It does not make card custody fully trustless because Cloudflare still performs the final off-chain dealing for this prototype.

Roadmap:

- v2: add Chainlink VRF or another verifiable randomness source as an extra seed input.
